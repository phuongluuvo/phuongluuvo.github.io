...
The technical companion to this file is [`README.md`](README.md) (tooling, local patches to
jemdoc, CI details).
...
published at **<https://phuongluuvo.me>** (and at `phuongluuvo.github.io`) with **GitHub
Pages**.
...
| **`tests/test_site.py`** | 44 automated checks on the *generated* site: no broken links, identical menu on every page, valid HTML and UTF-8, MathJax present, a well-formed publication list, and a news page that covers every publication. |
...
| `_site/` | **Generated.** The entire website — HTML, CSS, images, `CNAME`, `.nojekyll`. Ignored by git; it exists only after a build. |

### Where the generated site goes

`python build.py` writes the **whole** site into `_site/`: the HTML pages, a copy of the
stylesheet and the assets, `.nojekyll`, and `CNAME`. Pages left over from an earlier build
...
| **jemdoc** | **Nothing to install.** The exact version used by the site is vendored at `tools/jemdoc`, with five local fixes applied (see `README.md` §5). Do not `pip install` jemdoc. |
| **Perl** | Not needed. |
...
`www/files/` and `www/images/`, plus `.nojekyll` and `CNAME`. Nothing outside `_site/` is
touched.
...
4. Copies the generated `*.html` into the output folder (`_site/` unless `--out` says
   otherwise).
5. Copies `www/css/`, `www/files/`, `www/images/`, plus `CNAME`, and writes `.nojekyll`.
...
* One exception exists and should stay the only one: `CNAME` and `.nojekyll` (GitHub's own
  specified names).
...
- [ ] **Assets exist** in the output: `.nojekyll`, `CNAME`, `css/site.css`,
...
`CNAME` contains `phuongluuvo.me` and `build.py` copies it into whatever output folder you
build. To keep the domain: **Settings** → **Pages** → **Custom domain** → enter the domain →
**Save** → enable **Enforce HTTPS** (may take a few minutes). DNS must point at GitHub Pages
(four `A` records for an apex domain, or a `CNAME` record for `www`). To stop using it: delete
the `CNAME` file and clear the custom-domain field.

#### Using a different domain

There are two different "domains", and only one of them is yours to choose.

**The custom domain** — the address visitors type. This is already set to **phuongluuvo.me** by
the `CNAME` file. To move to another one:

1. Edit `CNAME` so that it holds only the new domain on one line — `example.com`, with no
   `http://` and no trailing slash — and commit it.
2. Point that domain's DNS at GitHub Pages: four `A` records for an apex domain
   (`185.199.108.153`, `185.199.109.153`, `185.199.110.153`, `185.199.111.153`) or one `CNAME`
   record for a `www` subdomain pointing at `<username>.github.io`.
3. In **Settings** → **Pages** → **Custom domain**, enter the new domain, **Save**, and enable
   **Enforce HTTPS** once the certificate has been issued.

**The `github.io` address** — `phuongluuvo.github.io` — comes from the repository name and
cannot be pointed anywhere else. Renaming the repository (**Settings** → **General** →
**Rename**) changes it, but this repository is a *user site* (`<username>.github.io`), so
renaming it would turn the address into `phuongluuvo.github.io/<new-name>` instead. Using a
custom domain is the cleaner answer.
...
1. **Never hand-edit generated files.** `build.py` writes the whole site into `_site/`, which
   ...
   * `_site/.nojekyll` and `_site/CNAME`.

   Their sources are, respectively: `www/*.jemdoc`, `www/css/site.css`, `www/files/`,
   `www/images/`, and `CNAME` in the repository root. `www/` is the only place to edit — with
   two exceptions: `www/publications.jemdoc` and `www/news.jemdoc` are both generated by
...
9. **Do not touch `tools/jemdoc` casually.** It is vendored upstream jemdoc + MathJax 0.7.3
   with **five** local fixes marked by a `LOCAL FIX` comment (see `README.md` §5). If you
   modify it, keep every `LOCAL FIX` intact and say so explicitly.
...
* This repository's tooling, vendored jemdoc and the five local patches: [`README.md`](README.md)
